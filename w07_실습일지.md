...
### *실습* 
 1) server(b) 초기화 
    - 우선 server(b)에 여전히 x윈도우가 설치되어 있어 `apt remove ubuntu-desktop`으로 x윈도우를 삭제하였다. ubuntu-desktop 역시 하나의 패키지 이므로 apt autoremove, apt remove를 사용함.
  > apt install mdadm 
  > 
